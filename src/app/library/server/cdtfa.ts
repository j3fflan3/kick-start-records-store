...
async function populateCaliforniaZips(): Promise<void> {
...
async function serverIsCaliforniaZip(postalCode: string): Promise<{ code: string } | undefined> {
  await populateCaliforniaZips();
  const caliZips = await redis.get<string>(CALIFORNIA_ZIPCODES);
  const aCaliZips: Array<{ code: string }> = Array.isArray(caliZips) ? caliZips : JSON.parse(caliZips as string);
  // Note: postalCode should be first 5 zip characters
  return aCaliZips.find((zip) => postalCode === zip.code);
}
