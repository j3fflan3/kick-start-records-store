...
interface CaptureOrderArgs {
  _paypal_capture_response: unknown;
  _subtotal: number;
  _shipping: number;
}

interface BuyNowCaptureOrderArgs {
  _paypal_capture_response: unknown;
  _subtotal: number;
}

// Order Related API Calls
async function payPalCreateOrder(createOrderArgs: unknown): Promise<unknown> {
  ...
  try {
    ...
    console.log(`create PayPal Order error: ${(err as Error).message}`);
    throw err;
  }
}

async function payPalCreateBuyNowOrder(createOrderArgs: unknown): Promise<unknown> {
  console.log(
  ...
  try {
    ...
    console.log(`create PayPal Buy Now Order error: ${(err as Error).message}`);
    throw err;
  }
}

async function payPalCaptureOrder(payPalOrderId: string): Promise<unknown> {
...
async function payPalUpdateOrder(captureOrderArgs: CaptureOrderArgs): Promise<{ data: unknown; error: unknown }> {
  console.log(
...
async function payPalUpdateBuyNowOrder(captureOrderArgs: BuyNowCaptureOrderArgs): Promise<{ data: unknown; error: unknown }> {
  console.log(
