...
interface ShoppingCartContextType {
  addToShoppingCart: (catalogId: string, is_anonymous: boolean, count?: number) => Promise<{ data: unknown; error: unknown }>;
  getShoppingCart: () => Promise<{ data: unknown }>;
  updateShoppingCart: (catalogId: string, count: number, email?: string | null) => Promise<{ data: unknown; error: unknown }>;
  setCartCount: React.Dispatch<React.SetStateAction<number>>;
  cartCount: number;
  setLocalCartIds: (value: shoppingCartKey | (() => shoppingCartKey)) => void;
  createLocalShoppingCart: (id?: string | null, is_anonymous?: boolean | null) => () => shoppingCartKey;
  localCartIds: shoppingCartKey;
  setOpenCart: React.Dispatch<React.SetStateAction<boolean>>;
  setCartItem: React.Dispatch<React.SetStateAction<unknown>>;
  setCartLink: React.Dispatch<React.SetStateAction<unknown>>;
  openCart: boolean;
  cartItem: unknown;
  cartLink: unknown;
  setCount: (products: Array<{ count: number }> | null) => void;
}
...
function ShoppingCartProvider({ children }: { children: React.ReactNode }) {
  const [localCartIds, setLocalCartIds] = useWebStorage<shoppingCartKey>(
    localCartKey,
    initialCart
  );
  const [cartCount, setCartCount] = useState(0);
  const [openCart, setOpenCart] = useState(false);
  const [cartItem, setCartItem] = useState<unknown>(null);
  const [cartLink, setCartLink] = useState<unknown>(null);
  ...
  useEffect(() => {
    async function mergeCarts(anonUserId: string, userId: string) {
      // console.log(
  ...
  function setCount(products: Array<{ count: number }> | null) {
  ...
  function createLocalShoppingCart(id: string | null = null, is_anonymous: boolean | null = null) {
    let date = new Date();
  ...
  async function addToShoppingCart(catalogId: string, is_anonymous: boolean, count = 1) {
    console.log(
  ...
  async function updateShoppingCart(catalogId: string, count: number, email: string | null = null) {
    let cnt = count;
    if (count > 10) {
      cnt = 10;
    }
