...
function BuyNowCheckout({ product, countries }: BuyNowCheckoutProps) {
  const [isPaying, setIsPaying] = useState(false);
  const [payWith, setPayWith] = useState("");
  const [tax, setTax] = useState(0);
  const [cardErrors, setCardErrors] = useState<Record<string, string>>({});
  const [payPalErrors, setPayPalErrors] = useState<Record<string, string>>({});
  const [billingInfo, setBillingInfo] = useState<Record<string, unknown>>({});
  const [countryOption, setCountryOption] = useState<CountryOption>({
    value: "US",
  ...
  const countriesOptions: CountryOption[] = countries.map((country) => {
    return {
                  ...
                  <div className="grid grid-cols-2 col-span-2 w-full my-2 mr-3.25">
                    ...
                    <div className="ml-2 col-span-1 mr-3.25">
                      <Select
                        ...
                        onChange={(option: SingleValue<CountryOption>) => option && setCountryOption(option)}
                        value={countryOption}
                      />
                    </div>
                  </div>
