...
import { getUSPSRates } from "@/src/app/library/server/usps";
import {
  InternalationRatesRequest,
  USBaseRatesRequest,
} from "@/src/app/library/model/usps";
...
function useShippingCalculator({
...
}: UseShippingCalculatorParams): { shippingCost: string; shippingCostCents: number; shippingError: string } {
  ...
  useEffect(() => {
    async function getBaseRates(sBaseRatesRequest: string, count: number) {
